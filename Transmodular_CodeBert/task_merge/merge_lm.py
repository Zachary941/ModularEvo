...
from task_eval.code_clone_eval import evaluate as evaluate_clone, Model as Model_clone
from task_eval.nl_code_search_eval import evaluate as evaluate_search, Model as Model_search

parser = argparse.ArgumentParser("Interface for merging PLMs on glue")
parser.add_argument("--language_model_name", type=str, default="codet5", help="name of the language model")
...
def get_merge_performance(args: argparse.Namespace, models_to_merge: list,  logger: logging.Logger,
    ...
    codebert_base = RobertaModel.from_pretrained(codebert_base_path,
                                                 from_tf=bool('.ckpt' in codebert_base_path),
                                                 config=config)
    base_state_dict = codebert_base.state_dict()
    model_code_clone = Model_clone(codebert_base,config,tokenizer).to('cuda')
    model_nl_code_search = Model_search(codebert_base,config,tokenizer).to('cuda')

    # set random seed to guarantee reproducibility
    set_random_seed(seed=0)
    # exclude parameter whose name matches "classification"
    merged_params = merging_method.get_merged_model(merged_model=model_code_clone,
...
if __name__ == "__main__":
    ...
    models_to_merge = []
    args.language_model_name = 'codebert-base'
    ...
    model_code_clone = Model_clone(codebert_base,config,tokenizer).to('cuda')
    model_nl_code_search = Model_search(codebert_base,config,tokenizer).to('cuda')
    print('======================code_clone model====================')
    model_ft1  = copy.deepcopy(model_code_clone)
    model_ft1.load_state_dict(torch.load(args.model_path1))
    print('======================nl_code_search model====================')
    model_ft2 = copy.deepcopy(model_nl_code_search)
    model_ft2.load_state_dict(torch.load(args.model_path2))

    models_to_merge.append(model_ft1)
    models_to_merge.append(model_ft2)
    ...
    # search for task_arithmetic
        ...
        #for alpha1 in [round(x * 0.1, 1) for x in range(0, 14)]:
        #    for alpha2 in [round(x * 0.1, 1) for x in range(0 , 14)]: 
        #for alpha1 in [0.0,1.0]:
             #for alpha2 in [0.0,1.0]:    
                #args.scaling_coefficients = [alpha1,alpha2]
                args.scaling_coefficients = [0.5,0.5]
                # dictionary
