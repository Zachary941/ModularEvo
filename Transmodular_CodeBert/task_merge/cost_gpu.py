...
def benchmark_with_deepsparse(model, inputs, export_path, batch_size=16, data_loader=None):
    ...
    # Create inference-only wrapper to avoid loss computation in ONNX export
    class InferenceWrapper(torch.nn.Module):
        def __init__(self, model):
            super(InferenceWrapper, self).__init__()
            self.model = model
            self.encoder = model.encoder if hasattr(model, 'encoder') else None
        
        def forward(self, input_ids, attention_mask=None):
            # Only forward the encoder part for inference
            if self.encoder is not None:
                outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
                return outputs[0]  # Return embeddings only
            else:
                # Fallback case
                with torch.no_grad():
                    outputs = self.model(input_ids, attention_mask, None)
                    if isinstance(outputs, tuple) and len(outputs) > 1:
                        return outputs[1]  # Return only logits
                    return outputs
    ...
    try:
        # Export model to ONNX format with lower opset version and more options
        dummy_input = (inputs[0], inputs[1]) if len(inputs) >= 2 else (inputs[0],)
        
        # Export directly with torch.onnx instead of ModuleExporter for more control
        torch.onnx.export(
            inference_model,
            dummy_input,
            export_path,
            ...
            input_names=['input_ids', 'attention_mask'] if len(inputs) >= 2 else ['input_ids'],
            output_names=['output'],
            dynamic_axes={
                'input_ids': {0: 'batch_size'},
                'attention_mask': {0: 'batch_size'} if len(inputs) >= 2 else None,
                'output': {0: 'batch_size'}
            }
        )
        ...
        input_data = [t.cpu().numpy() for t in inputs]
        ...
        if data_loader:
            # Fall back to PyTorch CPU benchmark if ONNX export fails
...
def main():
    ...
    # Tasks to evaluate
    tasks = [ "nl_code_search" ,"code_clone"]
    ...
    for task_type in tasks:
        ...
        deepsparse_ms_per_batch = benchmark_with_deepsparse(sparse_model, sample_inputs, export_path, batch_size, data_loader)
