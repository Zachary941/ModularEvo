...
if [[ ${TASK} == 'multi_task' ]]; then
  ...
  FULL_MODEL_TAG=${MODEL_TAG}_${DATA_TAG}_lr${LR}_bs${BS}_src${SRC_LEN}_trg${TRG_LEN}_pat${PATIENCE}_e${EPOCH}_wrr${WRR}
fi


if [[ ${SUB_TASK} == none ]]; then
  OUTPUT_DIR=${MODEL_DIR}_module/${TASK}/${FULL_MODEL_TAG}
else
  OUTPUT_DIR=${MODEL_DIR}_module/${TASK}/${SUB_TASK}/${FULL_MODEL_TAG}
fi

if [[ ${SUB_TASK} == none ]]; then
  CACHE_DIR=${WORKDIR}/data/${TASK}/cache_data
else
  CACHE_DIR=${WORKDIR}/data/${TASK}/${SUB_TASK}/cache_data
fi
